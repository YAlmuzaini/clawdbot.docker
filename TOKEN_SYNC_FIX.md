...
1. **Two Token Sources:**
   - Environment variable: `OPENCLAW_GATEWAY_TOKEN` (set in Coolify)
   - Config file: `/home/node/.openclaw/openclaw.json` (created by `onboard` command)
...
3. **When It Happens:**
   - Running `onboard` after setting `OPENCLAW_GATEWAY_TOKEN` in Coolify
...
- Automatically syncs `OPENCLAW_GATEWAY_TOKEN` from environment to config file on container startup
...
```bash
docker exec <container-name> sh -c 'node -e "const fs=require(\"fs\");const c=JSON.parse(fs.readFileSync(\"/home/node/.openclaw/openclaw.json\"));c.gateway.auth.token=process.env.OPENCLAW_GATEWAY_TOKEN;fs.writeFileSync(\"/home/node/.openclaw/openclaw.json\",JSON.stringify(c,null,2));"'
```

### Option 3: Set Token Before Onboard

1. Set `OPENCLAW_GATEWAY_TOKEN` in Coolify first
...
1. Set `OPENCLAW_GATEWAY_TOKEN` in Coolify environment variables
